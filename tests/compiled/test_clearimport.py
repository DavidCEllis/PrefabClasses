...
def test_cleared_prefab(module_name, class_letter):
    with prefab_compiler():
        example_import_clear = __import__(module_name, globals=globals(), locals=locals())
...
@pytest.mark.parametrize(
    "module_name",
    ["example_import_noclear_dynamic", "example_import_noclear_dynamic2"]
)
@pytest.mark.usefixtures("compile_folder_modules")
...
def test_uncleared_prefab_makeprefab():
    with prefab_compiler():
        example_import_noclear = __import__(
            "example_import_noclear_makeprefab",
            globals=globals(),
            locals=locals()
        )
